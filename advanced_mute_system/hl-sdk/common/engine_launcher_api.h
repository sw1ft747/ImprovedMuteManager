...
typedef struct engine_api_s
{
	...
	void	( *Cbuf_AddText )				( const char *text ); // append cmd at end of buf
	void	( *Cbuf_InsertText )			( const char *text ); // insert cmd at start of buf
	void	( *Cmd_AddCommand )				( const char *cmd_name, void ( *funcname )( void ) );
	int		( *Cmd_Argc )					( void );
	char	*( *Cmd_Args )					( void );
	...
	int		( *GetGameInfo )				( struct GameInfo_s *pGI, const char *pszChannel );
	void	( *GameSetBackground )			( int bBack );
	void	( *GameSetState )				( int iState );
	void	( *GameSetSubState )			( int iState );
	int		( *GetPauseState )				( void );
	int		( *Host_Frame )					( float time, int iState, int *stateInfo );
	void	( *Host_GetHostInfo )			( float *fps, int *nActive, int *nSpectators, int *nMaxPlayers, const char *pszMap );
	void	( *Host_Shutdown )				( void );
	int		( *Game_Init )					( const char *lpCmdLine, unsigned const char *pMem, int iSize, struct exefuncs_s *pef, void *, int );
